...
(function (w) {
  ...
  V.home = function () {
    ...
    /* ---------- 2. три списка, которые листаются вбок ---------- */
    ...
    /* тревожная карточка остаётся сверху и не листается */
    if (due.length) {
    ...
    /* --- страница 1: сколько накопилось процентов на сегодня --- */
    var p1 = '';
    if (owed.length) {
      p1 += '<div class="deck-sum">Уже набежало и не выплачено: <b class="num">' +
        V.sum(s.interestDue, s.cur.interestDue) + '</b></div><div class="list" id="owed-list">';
    ...
    /* --- страница 2: ближайшие платежи по графику --- */
    var p2 = '';
    if (due.length) {
    ...
    /* --- страница 3: сколько приносит каждый --- */
    var p3 = '';
    if (perClient.length) {
      p3 += '<div class="deck-sum">Итого <b class="num">' + V.sum(s.perMonth, s.cur.perMonth) +
        '</b> в месяц, ' + V.sum(s.dailyAccrual, s.cur.daily) + ' в день</div><div class="list" id="income-list">';
