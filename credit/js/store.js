...
(function (w) {
  ...
  var DB = {
    ...
    /* Загрузка CSV. Понимает два вида таблиц:
       — простая:  Имя;Телефон;Сумма;Дата выдачи;Ставка;Период;Срок в днях;Заметка
       — свою выгрузку (в ней есть столбцы «Валюта» и «Остаток долга»)          */
    importCSV: function (text) {
      ...
      var isExport = head.indexOf('валюта') >= 0 && head.indexOf('остаток') >= 0;
      ...
      var self = this, added = 0, skipped = 0;

      function cell(row, i) { return String(row[i] == null ? '' : row[i]).trim().replace(/^"|"$/g, ''); }
      function period(v) {
        v = String(v || '').toLowerCase().replace('в ', '').trim();
        return per[v.slice(0, 6)] || per[v.slice(0, 3)] || self.data.settings.defaultPeriod;
      }

      for (var i = start; i < lines.length; i++) {
        var c = lines[i].split(sep).map(function (x) { return x.trim().replace(/^"|"$/g, ''); });
        if (!c[0]) continue;
        var name = c[0], phone = cell(c, 1), principal = U.num(c[2]);
        if (!principal) { skipped++; continue; }
        var loan;

        if (isExport) {
          var issued = self.parseDate(c[4]) || U.today();
          var paid = U.num(c[9]);
          loan = {
            principal: principal,
            currency: FX.fromSymbol(cell(c, 3) || self.data.settings.currency),
            issuedAt: issued,
            model: 'simple',
            rate: U.num(c[5]),
            ratePeriod: period(c[6]),
            payMode: /ежемес/i.test(cell(c, 7)) ? 'monthly' : 'end',
            dueAt: self.parseDate(c[8]),
            penaltyRate: self.data.settings.penaltyRate || 0,
            status: /закрыт/i.test(cell(c, 11)) ? 'closed' : 'active',
            note: ''
          };
          /* в таблице нет дат платежей — переносим одной строкой на сегодня */
          if (paid > 0) loan.payments = [{ id: U.uid(), date: U.today(), amount: paid, note: 'перенесено из таблицы' }];
        } else {
          var iss = self.parseDate(c[3]) || U.today();
          var term = U.num(cell(c, 6));   // пустой столбец = срок не назначен
          loan = {
            principal: principal,
            currency: self.data.settings.currency,
            issuedAt: iss,
            model: 'simple',
            rate: c[4] != null && c[4] !== '' ? U.num(c[4]) : self.data.settings.defaultRate,
            ratePeriod: period(c[5]),
            payMode: self.data.settings.defaultPayMode || 'monthly',
            dueAt: term ? U.addDays(iss, term) : null,
            penaltyRate: self.data.settings.penaltyRate || 0,
            note: cell(c, 7)
          };
        }

        var cl = self.clientByName(name) || self.addClient({ name: name, phone: phone });
        if (phone && !cl.phone) cl.phone = phone;
        loan.clientId = cl.id;
        self.addLoan(loan);
        added++;
      }
      if (!added && skipped) throw new Error('в строках не нашлась сумма займа');
      return added;
    },
