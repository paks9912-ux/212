...
(function (w) {
  ...
  var MONTHS = ['январ', 'феврал', 'март', 'апрел', 'ма[йя]', 'июн', 'июл', 'авгус', 'сентябр', 'октябр', 'ноябр', 'декабр'];
  ...
  function monthIndex(stem) {
    for (var i = 0; i < MONTHS.length; i++) if (new RegExp('^' + MONTHS[i]).test(stem)) return i + 1;
    return null;
  }
  ...
  function pairedRange(t) {
    ...
    if (!m1 && !m2) return null;                       // «с 3 по 7» без месяца разберут points()
  ...
  function parseBudget(t) {
    ...
    var m = new RegExp('(?:до|не (?:более|дороже|больше)|бюджет[а-яa-z]*|в пределах|максимум|макс\\b|около|порядка|за)\\s*(\\d[\\d\\s]{0,11})\\s*(к\\b|тыс[а-яa-z]*|млн|000)?\\s*(сум[а-яa-z]*|сом[а-яa-z]*|руб[а-яa-z]*|\\$|долл[а-яa-z]*|usd|у\\.?е\\.?)?').exec(t);
    ...
    var mult = m[2] || '';
    if (/^к\b|тыс/.test(mult)) amount *= 1000;
  ...
  var SIGNALS = {
    ...
    lateCheckOut:   /поздн[а-яa-z]* выезд|выехать позже|выезд в \d{1,2}|попозже съехать|вылет вечером/,
    ...
    pets:           /с котом|с кошк|с собак|с животн|питомц|щенок|котенок|померан|хорьк|с птиц/,
    ...
    complaint:      /сверл|шумят|не дают спать|не работает|сломал|не включается|нет воды|нет света|холодно|жарко|грязн|не убрано|запах|шум[а-яa-z]* соседи|тараканы|клопы|не могу попасть|не открывается дверь|плохо пахнет/,
    damage:         /разбил|сломали|залил|затопил|испортил|пятно|порвал/,
    keys:           /потерял ключ|захлопнул|ключи внутри|не могу открыть замок|забыл ключ/,
    payment:        /как оплатить|оплата|перевод|карта|наличк|наличными|click|payme|humo|uzcard|реквизит/,
    prepayNo:       /без предоплат|оплачу на месте|предоплату не делаю|сначала посмотрю|не буду платить заранее/,
    fraud:          /переведи на друг[а-яa-z]* карт|карта не моя|оплатит друг|я вышлю чек|скрин оплаты|верните излишек|ошибочно перевел|перевел лишн|лишн[а-яё]* (?:перевод|сумм|деньг)|верн(?:ите|уть) (?:на )?(?:друг|иную|мою) |на другую карту|на другой счет|отправьте на мою карту|пришлите фото вашего паспорта|дайте данные карты|3d-код|смс-код|код из смс/,
    ...
    aggression:     /хамств|вы охренел|обман[а-яa-z]*|кинул|мошенник|в суд подам|напишу жалобу|верните немедленно|бред|дурак|идиот|позор/,
