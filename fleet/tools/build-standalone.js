...
const NAMES = ['util', 'store', 'calc', 'views', 'detail', 'app'];
const JS = NAMES.map(n => read('js/' + n + '.js'));
const css = read('css/app.css');

// сервис-воркера в одном файле нет — регистрировать нечего
const appIdx = NAMES.indexOf('app');
JS[appIdx] = JS[appIdx].replace(
...
<nav class="tabbar">
  ...
  <a href="#/log"   data-tab="log"><span id="i-log"></span><span>Журнал</span></a>
...
<script>
${JS.join('\n')}
['home','units','log','more'].forEach(function(k){
...
if (artifactIdx >= 0) {
  ...
  fs.writeFileSync(out, `<title>Автопарк</title>\n${body}\n`);
  console.log('собрано для Artifact →', out, (fs.statSync(out).size / 1024).toFixed(0) + ' КБ');
} else {
  const out = path.join(root, 'avtopark-standalone.html');
  fs.writeFileSync(out, `<!DOCTYPE html>
<html lang="ru">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover,user-scalable=no">
<title>Автопарк — техника и ГСМ</title>
